...
abstract class KeyRecovery {
  /// Clients create an Envelope at registration with this function.
  ...
  /// Clients recover their Envelope during login with this function.
...
class KeyRecoveryImpl implements KeyRecovery {
  ...
  /// Deterministically generates a key pair from the [seed] using the DST
  /// "OPAQUE-HashToScalar".
  Future<KeyPair> deriveAuthKeyPair(Bytes seed) async {
    // https://www.ietf.org/archive/id/draft-irtf-cfrg-opaque-07.html#section-6.4.2-4
  ...
  Future<StoreResult> store({
    ...
    final envelopeNonce =
        testEnvelopeNonce ?? await randomSeed(suite.constants.Nn);
